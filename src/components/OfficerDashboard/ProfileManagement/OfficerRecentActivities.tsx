...
const OfficerRecentActivities: React.FC<ActivityProps> = ({
      ...
      renderItem: (rental: any) => (
        ...
        >
          <ListItemText
            ...
            secondary={
              <Box sx={{ mt: 0.3, color: theme.palette.text.secondary }}>
                <div>Kitap: {rental.kitaplar?.kitap_adi || "Bilinmiyor"}</div>
                <div>
                  Yazar: {rental.kitaplar?.yazarlar?.isim || "Bilinmiyor"}
                </div>
                <div>Kiralama Tarihi: {formatDate(rental.kiralama_tarihi)}</div>
              </Box>
            }
          />
        </ListItem>
      ...
      renderItem: (returned: any) => (
        ...
        >
          <ListItemText
            ...
            secondary={
              <Box sx={{ mt: 0.3, color: theme.palette.text.secondary }}>
                <div>Kitap: {returned.kitaplar?.kitap_adi || "Bilinmiyor"}</div>
                <div>
  ...
  return (
    <Box sx={{ width: "100%", py: 3, px: { xs: 1, md: 3 } }}>
      <Grid
        container
        spacing={4}
      ...
      >
        {/* Üstte 3 kutu */}
        {activities.slice(0, 3).map(({ title, items, renderItem }) => (
          <Grid key={title}>
            <Paper
              elevation={6}
              sx={{
        ...
        {/* Altta 2 kutu */}
        {activities.slice(3).map(({ title, items, renderItem }) => (
          <Grid key={title}>
            <Paper
              elevation={6}
              sx={{
