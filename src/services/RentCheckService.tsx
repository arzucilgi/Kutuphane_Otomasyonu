...
// ) => {
//   try {
//     // 1. Aynı kitabı daha önce almış mı (ve iade etmemiş mi)?
...
//     // 2. Aktif kiralama sayısını kontrol et
...
export const rentBookIfAllowed = async (
  ...
  try {
    ...
    // 5. Kitap stok bilgisini güncelle (stok - 1)
    ...
    // ✅ 6. Kullanıcının aktif kitap sayısını artır (aktif_kiralik_kitap_sayisi = count + 1)
    const { error: updateUserError } = await supabase
      .from("kullanicilar")
      .update({ aktif_kiralik_kitap_sayisi: (count ?? 0) + 1 })
      .eq("id", user_id);
